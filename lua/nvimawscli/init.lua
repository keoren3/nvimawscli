...
function M.launch(command_service)
	...
	print("launching dashboard", command_service)
	if command_service then
		local status, service = pcall(require, "nvimawscli." .. command_service)
		if status then
			-- TODO: services location should be based on its type, like menu always top left but other services can be in place
